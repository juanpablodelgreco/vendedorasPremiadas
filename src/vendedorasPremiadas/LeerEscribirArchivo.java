...
public class LeerEscribirArchivo {
	...
	public static int Leer(String path, Vendedora[] vendedoras, int _cantidadVendedoras) throws FileNotFoundException {
		int cantVendedoras = _cantidadVendedoras,
		...
		for(int i=0 ; i<cantVendedoras ; i++) {
			...
			v.id = i+1;
			v.ventasConsecutivas = sc.nextInt();
			ventas = new int[v.ventasConsecutivas];
			for(int j=0 ; j<v.ventasConsecutivas ; j++) {
				ventas[j] = sc.nextInt();
			}
	...
	public static void Escribir(String path, Vendedora vendedora_premiada, String desempate) throws FileNotFoundException {
